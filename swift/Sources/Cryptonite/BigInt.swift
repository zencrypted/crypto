...
//===--- BigInt.swift -----------------------------------------------------===//
//
// Adapted from the Swift Standard Library Prototype:
// https://github.com/swiftlang/swift/blob/main/test/Prototypes/BigInt.swift
//
//===----------------------------------------------------------------------===//
...
extension BigInt {
    ...
    /// Initializes BigInt from a Little-Endian byte array.
    ...
    /// Returns Little-Endian byte array representation.
    public func toLEBytes() -> [UInt8] {
        ...
        // Remove trailing zeros? Standard crypto usually expects fixed size, but BigInt is dynamic.
    ...
    public var bitLength: Int {
        var len = self.words.count
        if len == 0 { return 0 }
        
        var i = len - 1
        while i >= 0 && self.words[i] == 0 {
            i -= 1
        }
        if i < 0 { return 0 }
        
        let highWord = self.words[i]
        // leadingZeroBitCount is available on UInt (FixedWidthInteger)
        return (i * UInt.bitWidth) + (UInt.bitWidth - highWord.leadingZeroBitCount)
    }
}
  ...
  where Word.Magnitude == Word
{
  ...
  public init<T : BinaryInteger>(_ source: T) {
    ...
    // assert(wordRatio != 0) // May be 0 on 32-bit if Word is UInt64? No, standard BigInt uses UInt.
    
    // Simplification for standard usage where Word == UInt
    if Word.bitWidth == UInt.bitWidth {
       for word in source.words {
           _data.append(Word(truncatingIfNeeded: word))
       }
    } else {
        // Fallback for disparate word sizes (generic implementation)
         for var sourceWord in source.words {
             let ratio = Swift.max(1, UInt.bitWidth / Word.bitWidth)
             for _ in 0..<ratio {
  ...
  mutating func _standardize(source: String = #function) {
    // defer { _checkInvariants(source: source + " >> _standardize()") } // Performance: Disable checks in prod
    while _data.last == 0 {
      _data.removeLast()
    }
  ...
  func _checkInvariants(source: String = #function) {
      // Disabled for performance in release
//    if _data.isEmpty {
...
//    assert(_data.last != 0, "\(source): extra zeroes on _data")
  ...
  mutating func _unsignedSubtract(_ rhs: Word) {
    // precondition(_data.count > 1 || _data[0] > rhs) // Allow zero result
  ...
  public var words: [UInt] {
    // assert(UInt.bitWidth % Word.bitWidth == 0) // May not be true for all generic Words
    let twosComplementData = _dataAsTwosComplement()
    
    // Simplification when Word == UInt
    if Word.bitWidth == UInt.bitWidth {
        // Safe cast if Word is UInt
        return twosComplementData.map { UInt(truncatingIfNeeded: $0) }
    }
    
    // Generic logic for disparate sizes (omitted for brevity unless causing issues)
    // Minimal fallback:
    return twosComplementData.map { UInt($0) }
  }
  ...
  public static func ^=(lhs: inout _BigInt, rhs: _BigInt) {
    ...
    // Capture the common count BEFORE modifying lhsTemp
    let commonCount = Swift.min(lhsTemp.count, rhsTemp.count)
  ...
  public static func <<= <RHS : BinaryInteger>(lhs: inout _BigInt, rhs: RHS) {
    ...
    for i in ((extraWords + 1)..<lhs._data.count).reversed() {
        if i - 1 >= 0 {
            lhs._data[i] = lhs._data[i] << highOffset | lhs._data[i - 1] >> lowOffset
        } else {
