...
@usableFromInline struct DirectoryAbstractService_PagedResultsRequest_newRequest_Sequence: DERImplicitlyTaggable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.sequence(root, identifier: identifier) { nodes in
            ...
            let reverse: Bool? = try DER.optionalExplicitlyTagged(&nodes, tagNumber: 1, tagClass: .contextSpecific) { node in return try Bool(derEncoded: node) }
            let unmerged: Bool? = try DER.optionalExplicitlyTagged(&nodes, tagNumber: 2, tagClass: .contextSpecific) { node in return try Bool(derEncoded: node) }
