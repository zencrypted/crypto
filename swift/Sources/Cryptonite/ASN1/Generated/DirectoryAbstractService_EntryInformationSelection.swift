...
@usableFromInline struct DirectoryAbstractService_EntryInformationSelection: DERImplicitlyTaggable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.set(root, identifier: identifier) { nodes in
            var attributes: DirectoryAbstractService_EntryInformationSelection_attributes_Choice? = nil
var peek_attributes = nodes
if let next = peek_attributes.next(), next.identifier == DirectoryAbstractService_EntryInformationSelection_attributes_Choice.defaultIdentifier {
    attributes = try DirectoryAbstractService_EntryInformationSelection_attributes_Choice(derEncoded: &nodes)
}
            let infoTypesVal = try Int(derEncoded: &nodes)
            let infoTypes = DirectoryAbstractService_EntryInformationSelection_infoTypes_IntEnum(rawValue: infoTypesVal)
...
if let next = peek_contextSelection.next(), next.identifier == DirectoryAbstractService_ContextSelection.defaultIdentifier {
    contextSelection = try DirectoryAbstractService_ContextSelection(derEncoded: &nodes)
            ...
            var familyReturn: DirectoryAbstractService_FamilyReturn? = nil
var peek_familyReturn = nodes
if let next = peek_familyReturn.next(), next.identifier == DirectoryAbstractService_FamilyReturn.defaultIdentifier {
    familyReturn = try DirectoryAbstractService_FamilyReturn(derEncoded: &nodes)
}
