...
@usableFromInline struct DirectoryAbstractService_ServiceControls: DERImplicitlyTaggable, Sendable {
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.set(root, identifier: identifier) { nodes in
            let options: DirectoryAbstractService_ServiceControlOptions? = try DER.optionalExplicitlyTagged(&nodes, tagNumber: 0, tagClass: .contextSpecific) { node in return try DirectoryAbstractService_ServiceControlOptions(derEncoded: node) }
            let priorityVal = try Int(derEncoded: &nodes)
            let priority = DirectoryAbstractService_ServiceControls_priority_IntEnum(rawValue: priorityVal)
            ...
            let scopeOfReferralVal = try Int(derEncoded: &nodes)
            let scopeOfReferral = DirectoryAbstractService_ServiceControls_scopeOfReferral_IntEnum(rawValue: scopeOfReferralVal)
