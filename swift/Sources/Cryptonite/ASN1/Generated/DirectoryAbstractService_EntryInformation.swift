...
@usableFromInline struct DirectoryAbstractService_EntryInformation: DERImplicitlyTaggable, Sendable {
    ...
    @usableFromInline var name: InformationFramework_Name
    ...
    @inlinable init(name: InformationFramework_Name, fromEntry: Bool?, information: [DirectoryAbstractService_EntryInformation_information_Choice]?, incompleteEntry: Bool?, partialNameResolution: Bool?) {
        self.name = name
    ...
    @inlinable init(derEncoded root: ASN1Node,
        ...
        self = try DER.sequence(root, identifier: identifier) { nodes in
            let name: InformationFramework_Name = try InformationFramework_Name(derEncoded: &nodes)
            ...
            let incompleteEntry: Bool? = try DER.optionalExplicitlyTagged(&nodes, tagNumber: 3, tagClass: .contextSpecific) { node in return try Bool(derEncoded: node) }
            let partialNameResolution: Bool? = try DER.optionalExplicitlyTagged(&nodes, tagNumber: 4, tagClass: .contextSpecific) { node in return try Bool(derEncoded: node) }
